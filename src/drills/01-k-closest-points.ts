...
class MaxPointHeap {
  ...
  /** guarded push: admit either when below k or when new value is less than max. */
  push(e: Entry): void {
    const root = this.peek();
    let shouldPopAndSift = this.size < this.k;

    if (!shouldPopAndSift && root && e.d < root.d) {
      this.pop();
      shouldPopAndSift = true;
    }
